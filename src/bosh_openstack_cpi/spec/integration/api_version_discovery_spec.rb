...
describe Bosh::OpenStackCloud::Cloud do
  ...
  describe 'Glance version discovery' do
    context 'when only V1 is available' do
      ...
      it 'CPI uses V1' do
        expect(@config.create_cpi.glance.class.to_s).to start_with('Fog::OpenStack::Image::V1')
      end
    end

    context 'when V2 is available' do
      it 'CPI uses V2' do
        expect(@config.create_cpi.glance.class.to_s).to start_with('Fog::OpenStack::Image::V2')
      end
    end
  end
  ...
  describe 'Cinder version discovery' do
    context 'when only V1 is available' do
      ...
      it 'CPI uses V1' do
        expect(@config.create_cpi.volume.class.to_s).to start_with('Fog::OpenStack::Volume::V1')
      end
    end

    context 'when V2 is available' do
      it 'CPI uses V2' do
        expect(@config.create_cpi.volume.class.to_s).to start_with('Fog::OpenStack::Volume::V2')
      end
    end
  end
